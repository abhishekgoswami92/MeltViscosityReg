...
A_feat = to_feature_vector(A_param)
B_feat = to_feature_vector(B_param)
T_feat = to_feature_vector(T_param)


x_log = estimate_x_from_features(A_feat, B_feat, T_feat)


T_pred = (1 - x_log)*A_visc+ x_log*B_visc
...
plt.legend()
plt.savefig(f"{add}Validation.pdf")
